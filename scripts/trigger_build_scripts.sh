...
then
    ...
    LATEST_BUILD_FOLDER=$(aws s3 ls s3://dss-artifacts/builds/DSS/ | grep -Eo '[0-9]{1,4}' | tail -1)
    echo "Executing build_gcc.sh"
    ./scripts/build_gcc.sh
    echo "Downloading aws-sdk RPM to the directory"
    aws s3 cp --recursive s3://$BUCKET_NAME/builds/DSS/$LATEST_BUILD_FOLDER/ dss-ansible/artifacts/ --exclude "*" --include "aws-sdk-cpp-*.rpm"
...
then
    ...
    aws s3 cp --recursive s3://$BUCKET_NAME/builds/DSS/$LATEST_BUILD_FOLDER/ dss-ansible/artifacts/ --exclude "*" --include "dss-gcc510-*.rpm"
elif [[ $RUN_SCRIPT == "None" ]]
then
    echo "Don't Run Any Script & download both the RPM Files to the Directory"
    echo "Fetching the latest build folder path"
    LATEST_BUILD_FOLDER=$(aws s3 ls s3://dss-artifacts/builds/DSS/ | grep -Eo '[0-9]{1,4}' | tail -1)
    aws s3 cp --recursive s3://$BUCKET_NAME/builds/DSS/$LATEST_BUILD_FOLDER/ dss-ansible/artifacts/ --exclude "*" --include "*.rpm"
elif [[ $RUN_SCRIPT == "ALL" ]]
then
